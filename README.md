...
- **策略回测** — 历史K线回放，复用实盘AI决策链，资金曲线、绩效指标、交易记录全链路验证
...
### 策略回测 (`web/src/components/backtest/` + `backtest/`)

历史K线驱动的策略验证系统，复用 `kernel/` 实盘决策链：
- 多币种同时回测，Binance 历史数据自动拉取
- AI 决策全链路回放（选币→指标→LLM→模拟下单）
- 资金曲线 + K线图 + 交易标记可视化
- 绩效指标：总收益率、夏普比率、最大回撤、胜率
- 暂停/恢复/停止/删除，SSE 实时推送运行状态
- 详见 [docs/backtest.md](docs/backtest.md)
...
- **策略管理** — 可视化编辑器 + 公开策略市场
- **AI 竞赛** — 多交易员实时排名
- **策略回测** — 回测配置、运行管理、资金曲线图表、交易记录、AI决策日志
- **配置管理** — AI 模型、交易所、钱包
