...
export const translations = {
  en: {
    ...
    // Footer
    footerTitle: 'AiT - AI Trading System',
    ...
    ai500Description:
      'AI500 candidates are produced from the local Binance Futures SnapshotStore; no extra data-source API endpoint is required',
    oiTopDescription:
      'Open interest rankings are produced from the local Binance Futures SnapshotStore',
    information: 'Information',
    signalSourceInfo1:
      '• Signal sources now read from the backend SnapshotStore instead of per-user external URLs',
    signalSourceInfo2:
      '• When creating traders, you can choose whether to use these signal sources',
    signalSourceInfo3:
      '• Binance Futures market data is fetched once by the backend and reused by strategies',
    ...
    proxyURLHelp:
      'Set a dedicated network proxy for this exchange. Supports HTTP/HTTPS/SOCKS5',
    optional: 'optional',
    ...
    heroDescription:
      'AiT is the future standard for AI trading — an open, community-driven agentic trading OS. Supporting Binance, Aster DEX and other exchanges, self-hosted, multi-agent competition, let AI automatically make decisions, execute and optimize trades for you.',
    poweredBy: 'Powered by Aster DEX and Binance.',

    // Landing Page CTA
    readyToDefine: 'Ready to define the future of AI trading?',
    startWithCrypto:
      'Starting with crypto markets, expanding to TradFi. AiT is the infrastructure of AgentFi.',
    ...
    howToStart: 'How to Get Started with AiT',
    fourSimpleSteps:
    ...
    riskWarningText:
      'Dev branch is unstable, do not use funds you cannot afford to lose. AiT is non-custodial, no official strategies. Trading involves risks, invest carefully.',
    ...
    ai500ApiNotConfigured:
      'AI500 local snapshot pool has no candidates yet (check SnapshotStore warm-up and Binance connectivity)',
    apiConnectionTimeout:
      'SnapshotStore is still warming up or returned empty data',
    noCustomCoinsAndApiFailed:
      'No custom coins configured and the local snapshot candidate pool is empty',
    solutions: 'Solutions:',
    setCustomCoinsInConfig: 'Set custom coin list in trader configuration',
    orConfigureCorrectApiUrl:
      'Or wait for SnapshotStore warm-up and verify Binance connectivity',
    orDisableAI500Options:
      'Or disable "Use AI500 Local Snapshot Pool" and "Use OI Top" options',
    signalSourceNotConfigured: 'Snapshot Candidate Pool Empty',
    signalSourceWarningMessage:
      'You have traders that enabled "Use AI500 Local Snapshot Pool" or "Use OI Top", but SnapshotStore has not produced candidates yet. Candidate coins may stay at 0 until the local snapshot warms up.',
    configureSignalSourceNow: 'Open Strategy Settings',

    // FAQ Page
    faqTitle: 'Frequently Asked Questions',
    faqSubtitle: 'Find answers to common questions about AiT',
    ...
    // ===== GETTING STARTED =====
    faqWhatIsAiT: 'What is AiT?',
    faqWhatIsAiTAnswer:
      'AiT is an open-source AI-powered trading operating system for cryptocurrency and US stock markets. It uses large language models (LLMs) like DeepSeek, GPT, Claude, Gemini to analyze market data and make autonomous trading decisions. Key features include: multi-AI model support, multi-exchange trading, and visual strategy builder.',

    faqHowDoesItWork: 'How does AiT work?',
    faqHowDoesItWorkAnswer:
      'AiT works in 5 steps: 1) Configure AI models and exchange API credentials; 2) Create a trading strategy (coin selection, indicators, risk controls); 3) Create a "Trader" combining AI model + Exchange + Strategy; 4) Start the trader - it will analyze market data at regular intervals and make buy/sell/hold decisions; 5) Monitor performance on the dashboard. The AI uses Chain of Thought reasoning to explain each decision.',

    faqIsProfitable: 'Is AiT profitable?',
    faqIsProfitableAnswer:
      'AI trading is experimental and NOT guaranteed to be profitable. Cryptocurrency futures are highly volatile and risky. AiT is designed for educational and research purposes. We strongly recommend: starting with small amounts (10-50 USDT), never investing more than you can afford to lose, thoroughly testing before live trading, and understanding that past performance does not guarantee future results.',
    ...
    faqStopLossTakeProfit: 'Does AiT support stop-loss and take-profit?',
    ...
    faqMultipleTradersAnswer:
      'Yes! AiT supports running 20+ concurrent traders. Each trader can have different: AI model, exchange account, strategy, decision interval. Use this to A/B test strategies, compare AI models, or diversify across exchanges. Monitor all traders on the Competition page.',
    ...
    faqPortInUseAnswer:
      'Check what\'s using the port: "lsof -i :8080" (macOS/Linux) or "netstat -ano | findstr 8080" (Windows). Kill the process or change ports in .env: AiT_BACKEND_PORT=8081, AiT_FRONTEND_PORT=3001. Restart with "docker compose down && docker compose up -d".',
    ...
    faqBinancePositionModeAnswer:
      'Error: "Order\'s position side does not match user\'s setting". You\'re in One-way Mode but AiT requires Hedge Mode. Fix: 1) Close ALL positions first; 2) Binance Futures → Settings (gear icon) → Preferences → Position Mode → Switch to "Hedge Mode" (双向持仓); 3) Restart your trader.',

    faqBalanceShowsZero: 'Account balance shows 0',
    faqBalanceShowsZeroAnswer:
      'Funds are likely in Spot wallet, not Futures wallet. Solution: 1) In Binance, go to Wallet → Futures → Transfer; 2) Transfer USDT from Spot to Futures; 3) Refresh AiT dashboard. Also check: funds not locked in savings/staking products.',
    ...
    faqEncryptionDetails: 'What encryption does AiT use?',
    faqEncryptionDetailsAnswer:
      'AiT uses multiple encryption layers: 1) AES-256-GCM for database storage (API keys, secrets); 2) RSA-2048 for optional transport encryption (browser to server); 3) JWT for authentication tokens. Keys are generated during installation. Enable TRANSPORT_ENCRYPTION=true for HTTPS environments.',

    faqSecurityBestPractices: 'What are security best practices?',
    faqSecurityBestPracticesAnswer:
      'Recommended: 1) Use exchange API keys with IP whitelist and minimal permissions (Futures Trading only); 2) Use dedicated subaccount for AiT; 3) Enable TRANSPORT_ENCRYPTION for remote deployments; 4) Never share .env or data.db files; 5) Use HTTPS with valid certificates; 6) Regularly rotate API keys; 7) Monitor account activity.',

    faqCanAiTStealFunds: 'Can AiT steal my funds?',
    faqCanAiTStealFundsAnswer:
      'AiT is open-source (AGPL-3.0 license) - you can audit all code on GitHub. API keys are stored locally on YOUR machine, never sent to external servers. AiT only has the permissions you grant via API keys. For maximum safety: use API keys with trading-only permissions (no withdrawal), enable IP whitelist, use a dedicated subaccount.',
    ...
    faqCustomAIAPIAnswer:
      'Yes! AiT supports any OpenAI-compatible API. In Config → AI Models → Custom API: 1) Enter your API endpoint URL (e.g., https://your-api.com/v1); 2) Enter API key; 3) Specify model name. This works with self-hosted models, alternative providers, or Claude via third-party proxies.',

    faqAIHallucinations: 'What about AI hallucinations?',
    faqAIHallucinationsAnswer:
      'AI models can sometimes produce incorrect or fabricated information ("hallucinations"). AiT mitigates this by: 1) Providing structured prompts with real market data; 2) Enforcing JSON output format for decisions; 3) Validating orders before execution. However, AI trading is experimental - always monitor decisions and don\'t rely solely on AI judgment.',
    ...
    // ===== CONTRIBUTING =====
    faqHowToContribute: 'How can I contribute to AiT?',
    faqHowToContributeAnswer:
      'AiT is open-source and welcomes contributions! Ways to contribute: 1) Code - fix bugs, add features (check GitHub Issues); 2) Documentation - improve guides, translate; 3) Bug Reports - report issues with details; 4) Feature Ideas - suggest improvements. Start with issues labeled "good first issue". All contributors may receive airdrop rewards.',
    ...
    environmentCheck: {
      ...
      unsupportedDesc:
        'Open AiT over HTTPS (or http://localhost during development) and avoid insecure iframes/reverse proxies so the browser can enable Web Crypto.',
    ...
    strategyStudio: {
      ...
      promptCompactMode: 'Compact candidate data',
      promptCompactModeDesc:
        'Compacts candidate market data in the prompt to save tokens. Turn off to send full per-coin detail.',
      promptCompactOn: 'On',
      promptCompactOff: 'Off',
      promptCompactReloadHint:
        'Saved changes affect newly loaded trader configs; restart a running trader to apply immediately.',
    },
  ...
  zh: {
    ...
    ai500Description:
      'AI500 候选池来自本地 Binance Futures SnapshotStore，无需配置额外数据源 API 地址',
    oiTopDescription: '持仓量排行来自本地 Binance Futures SnapshotStore',
    information: '说明',
    signalSourceInfo1:
      '• 信号源统一读取后端 SnapshotStore，不再配置用户级外部 URL',
    signalSourceInfo2: '• 在创建交易员时可以选择是否使用这些信号源',
    signalSourceInfo3: '• Binance Futures 市场数据由后端统一拉取并复用给策略',
    editAIModel: '编辑AI模型',
    addAIModel: '添加AI模型',
    ...
    heroTitle2: 'Write the Trade.',
    heroDescription:
      'AiT 是 AI 交易的未来标准——一个开放、社区驱动的代理式交易操作系统。支持 Binance、Aster DEX 等交易所，自托管、多代理竞争，让 AI 为你自动决策、执行和优化交易。',
    poweredBy: '由 Aster DEX 和 Binance 提供支持。',

    // Landing Page CTA
    readyToDefine: '准备好定义 AI 交易的未来吗？',
    startWithCrypto:
      '从加密市场起步，扩展到 TradFi。AiT 是 AgentFi 的基础架构。',
    getStartedNow: '立即开始',
    viewSourceCode: '查看源码',
    ...
    riskWarningText:
      'dev 分支不稳定，勿用无法承受损失的资金。AiT 非托管，无官方策略。交易有风险，投资需谨慎。',
    ...
    ai500ApiNotConfigured:
      'AI500 本地快照池暂未产生候选币（请检查 SnapshotStore 预热和 Binance 连通性）',
    apiConnectionTimeout: 'SnapshotStore 仍在预热或返回数据为空',
    noCustomCoinsAndApiFailed: '未配置自定义币种且本地快照候选池为空',
    solutions: '解决方案：',
    setCustomCoinsInConfig: '在交易员配置中设置自定义币种列表',
    orConfigureCorrectApiUrl:
      '或者等待 SnapshotStore 预热并检查 Binance 连通性',
    orDisableAI500Options: '或者禁用"使用 AI500 本地快照池"和"使用 OI Top"选项',
    signalSourceNotConfigured: '快照候选池为空',
    signalSourceWarningMessage:
      '您有交易员启用了"使用 AI500 本地快照池"或"使用 OI Top"，但 SnapshotStore 暂未产出候选币。候选币数量可能在本地快照预热完成前保持为 0。',
    configureSignalSourceNow: '打开策略设置',

    // FAQ Page
    faqTitle: '常见问题',
    faqSubtitle: '查找关于 AiT 的常见问题解答',
    ...
    // ===== 入门指南 =====
    faqWhatIsAiT: 'AiT 是什么？',
    faqWhatIsAiTAnswer:
      'AiT 是一个开源的 AI 驱动交易操作系统，支持加密货币和美股市场。它使用大语言模型（LLM）如 DeepSeek、GPT、Claude、Gemini 来分析市场数据，进行自主交易决策。核心功能包括：多 AI 模型支持、多交易所交易、可视化策略构建器、回测系统。',

    faqHowDoesItWork: 'AiT 是如何工作的？',
    faqHowDoesItWorkAnswer:
      'AiT 分 5 步工作：1）配置 AI 模型和交易所 API 凭证；2）创建交易策略（币种选择、指标、风控）；3）创建"交易员"，组合 AI 模型 + 交易所 + 策略；4）启动交易员 - 它会定期分析市场数据并做出买入/卖出/持有决策；5）在仪表板上监控表现。AI 使用思维链（Chain of Thought）推理来解释每个决策。',

    faqIsProfitable: 'AiT 能盈利吗？',
    faqIsProfitableAnswer:
      'AI 交易是实验性的，不保证盈利。加密货币期货波动性大、风险高。AiT 仅用于教育和研究目的。我们强烈建议：从小额开始（10-50 USDT），不要投入超过承受能力的资金，在实盘交易前充分回测，并理解过去的表现不代表未来的结果。',
    ...
    faqSupportedAIModelsAnswer:
      'AiT 支持 7+ 种 AI 模型：DeepSeek（推荐性价比）、阿里云通义千问、OpenAI（GPT-5.2）、Anthropic Claude、Google Gemini、xAI Grok、Kimi（月之暗面）。您也可以使用任何 OpenAI 兼容的 API 端点。每个模型各有优势 - DeepSeek 性价比高，OpenAI 能力强但贵，Claude 擅长推理。',
    ...
    faqHyperliquidSetupAnswer:
      'Hyperliquid 是去中心化交易所，需要钱包认证。步骤：1）访问 app.hyperliquid.xyz；2）连接钱包；3）生成 API 钱包（推荐）或使用主钱包；4）复制钱包地址和私钥；5）在 AiT 中添加 Hyperliquid 交易所并填入凭证。无需 KYC，完全链上。',
    ...
    faqStopLossTakeProfit: 'AiT 支持止损止盈吗？',
    faqStopLossTakeProfitAnswer:
    ...
    faqMultipleTradersAnswer:
      '可以！AiT 支持运行 20+ 个并发交易员。每个交易员可以有不同的：AI 模型、交易所账户、策略、决策间隔。用于 A/B 测试策略、比较 AI 模型或跨交易所分散风险。在竞赛页面监控所有交易员。',
    ...
    faqPortInUseAnswer:
      '查看占用端口的进程：macOS/Linux 用 "lsof -i :8080"，Windows 用 "netstat -ano | findstr 8080"。终止进程或在 .env 中修改端口：AiT_BACKEND_PORT=8081、AiT_FRONTEND_PORT=3001。然后 "docker compose down && docker compose up -d" 重启。',
    ...
    faqBinancePositionModeAnswer:
      '错误："Order\'s position side does not match user\'s setting"。您处于单向持仓模式，但 AiT 需要双向持仓模式。修复：1）先平掉所有持仓；2）币安合约 → 设置（齿轮图标）→ 偏好设置 → 持仓模式 → 切换为"双向持仓"；3）重启交易员。',

    faqBalanceShowsZero: '账户余额显示 0',
    faqBalanceShowsZeroAnswer:
      '资金可能在现货钱包而非合约钱包。解决方案：1）在币安进入钱包 → 合约 → 划转；2）将 USDT 从现货划转到合约；3）刷新 AiT 仪表板。也检查：资金未被理财/质押产品锁定。',
    ...
    faqEncryptionDetails: 'AiT 使用什么加密？',
    faqEncryptionDetailsAnswer:
      'AiT 使用多层加密：1）AES-256-GCM 用于数据库存储（API 密钥、密钥）；2）RSA-2048 用于可选的传输加密（浏览器到服务器）；3）JWT 用于认证令牌。密钥在安装时生成。HTTPS 环境启用 TRANSPORT_ENCRYPTION=true。',

    faqSecurityBestPractices: '安全最佳实践是什么？',
    faqSecurityBestPracticesAnswer:
      '建议：1）使用带 IP 白名单和最小权限（仅合约交易）的交易所 API 密钥；2）为 AiT 使用专用子账户；3）远程部署启用 TRANSPORT_ENCRYPTION；4）切勿分享 .env 或 data.db 文件；5）使用有效证书的 HTTPS；6）定期轮换 API 密钥；7）监控账户活动。',

    faqCanAiTStealFunds: 'AiT 会盗取我的资金吗？',
    faqCanAiTStealFundsAnswer:
      'AiT 是开源的（AGPL-3.0 许可）- 您可以在 GitHub 审计所有代码。API 密钥存储在您的机器本地，从不发送到外部服务器。AiT 只有您通过 API 密钥授予的权限。为最大安全：使用仅交易权限（无提现）的 API 密钥，启用 IP 白名单，使用专用子账户。',
    ...
    faqCustomAIAPIAnswer:
      '可以！AiT 支持任何 OpenAI 兼容的 API。在配置 → AI 模型 → 自定义 API 中：1）输入 API 端点 URL（如 https://your-api.com/v1）；2）输入 API 密钥；3）指定模型名称。适用于自托管模型、替代提供商或通过第三方代理的 Claude。',

    faqAIHallucinations: 'AI 幻觉问题怎么办？',
    faqAIHallucinationsAnswer:
      'AI 模型有时会产生不正确或虚构的信息（"幻觉"）。AiT 通过以下方式缓解：1）提供带真实市场数据的结构化提示词；2）强制 JSON 输出格式；3）执行前验证订单。但 AI 交易是实验性的 - 始终监控决策，不要完全依赖 AI 判断。',
    ...
    // ===== 参与贡献 =====
    faqHowToContribute: '如何为 AiT 做贡献？',
    faqHowToContributeAnswer:
      'AiT 是开源项目，欢迎贡献！贡献方式：1）代码 - 修复 bug、添加功能（查看 GitHub Issues）；2）文档 - 改进指南、翻译；3）Bug 报告 - 详细报告问题；4）功能建议 - 提出改进意见。从标记为"good first issue"的问题开始。所有贡献者可能获得空投奖励。',
    ...
    environmentCheck: {
      ...
      unsupportedDesc:
        '请通过 HTTPS 或本机 localhost 访问 AiT，并避免嵌入不安全 iframe/反向代理，以符合浏览器的 Web Crypto 规则。',
    ...
    strategyStudio: {
      ...
      promptCompactMode: '压缩候选币数据',
      promptCompactModeDesc:
        '压缩 Prompt 中的候选币市场数据以节省 token；关闭后发送完整明细。',
      promptCompactOn: '开启',
      promptCompactOff: '关闭',
      promptCompactReloadHint:
        '保存后会写入策略配置；运行中的交易员需重启后立即生效。',
    },
    ...
    backtestChart: {
      candlestickTradeMarkers: 'K线 + 交易标记',
      close: '收盘',
      equityCurve: '资金曲线',
      interval: '周期',
      loadingKline: '加载K线中...',
  ...
  id: {
    ...
    // Footer
    footerTitle: 'AiT - Sistem Trading AI',
    ...
    ai500Description:
      'Kandidat AI500 berasal dari Binance Futures SnapshotStore lokal; endpoint API sumber data tambahan tidak diperlukan',
    oiTopDescription:
      'Peringkat open interest berasal dari Binance Futures SnapshotStore lokal',
    information: 'Informasi',
    signalSourceInfo1:
      '• Sumber sinyal membaca SnapshotStore backend, bukan URL eksternal per pengguna',
    signalSourceInfo2:
      '• Saat membuat trader, Anda dapat memilih apakah akan menggunakan sumber sinyal ini',
    signalSourceInfo3:
      '• Data Binance Futures diambil sekali oleh backend dan digunakan ulang oleh strategi',
    ...
    heroTitle2: 'Write the Trade.',
    heroDescription:
      'AiT adalah standar masa depan untuk trading AI — OS trading agensi yang terbuka dan didorong komunitas. Mendukung Binance, Aster DEX dan bursa lainnya, self-hosted, kompetisi multi-agen, biarkan AI secara otomatis membuat keputusan, mengeksekusi dan mengoptimalkan trading untuk Anda.',
    poweredBy: 'Didukung oleh Aster DEX dan Binance.',
    readyToDefine: 'Siap mendefinisikan masa depan trading AI?',
    startWithCrypto:
      'Dimulai dari pasar kripto, berkembang ke TradFi. AiT adalah infrastruktur AgentFi.',
    ...
    aitNotAnotherBot:
      "AiT bukan bot trading biasa, melainkan 'Linux' dari trading AI —",
    aitDescription1:
    ...
    step1Desc:
      'git clone https://github.com/Aixxww/AiT dan beralih ke branch dev untuk menguji fitur baru.',
    ...
    riskWarningText:
      'Branch dev tidak stabil, jangan gunakan dana yang tidak sanggup Anda rugi. AiT non-custodial, tanpa strategi resmi. Trading memiliki risiko, investasi dengan hati-hati.',
    ...
    ai500ApiNotConfigured:
      'Pool snapshot lokal AI500 belum memiliki kandidat (periksa warm-up SnapshotStore dan koneksi Binance)',
    apiConnectionTimeout:
      'SnapshotStore masih warm-up atau mengembalikan data kosong',
    noCustomCoinsAndApiFailed:
      'Tidak ada koin kustom dan pool kandidat snapshot lokal kosong',
    solutions: 'Solusi:',
    setCustomCoinsInConfig: 'Atur daftar koin kustom di konfigurasi trader',
    orConfigureCorrectApiUrl:
      'Atau tunggu warm-up SnapshotStore dan periksa koneksi Binance',
    orDisableAI500Options:
      'Atau nonaktifkan "AI500 Local Snapshot Pool" dan "OI Top"',
    signalSourceNotConfigured: 'Pool Kandidat Snapshot Kosong',
    signalSourceWarningMessage:
      'Trader mengaktifkan "AI500 Local Snapshot Pool" atau "OI Top", tetapi SnapshotStore belum menghasilkan kandidat. Jumlah kandidat dapat tetap 0 sampai snapshot lokal selesai warm-up.',
    configureSignalSourceNow: 'Buka Pengaturan Strategi',

    // FAQ Page
    faqTitle: 'Pertanyaan yang Sering Diajukan',
    faqSubtitle: 'Temukan jawaban untuk pertanyaan umum tentang AiT',
    ...
    faqWhatIsAiT: 'Apa itu AiT?',
    faqWhatIsAiTAnswer:
      'AiT adalah sistem operasi trading bertenaga AI open-source untuk pasar kripto dan saham AS. Ia menggunakan model bahasa besar (LLM) seperti DeepSeek, GPT, Claude, Gemini untuk menganalisis data pasar dan membuat keputusan trading secara otonom. Fitur utama: dukungan multi-model AI, trading multi-bursa, dan pembangun strategi visual.',
    faqHowDoesItWork: 'Bagaimana cara kerja AiT?',
    faqHowDoesItWorkAnswer:
      'AiT bekerja dalam 5 langkah: 1) Konfigurasi model AI dan kredensial API bursa; 2) Buat strategi trading (pemilihan koin, indikator, kontrol risiko); 3) Buat "Trader" menggabungkan Model AI + Bursa + Strategi; 4) Mulai trader - dia akan menganalisis data pasar secara berkala dan membuat keputusan beli/jual/tahan; 5) Pantau performa di dasbor.',
    faqIsProfitable: 'Apakah AiT menguntungkan?',
    faqIsProfitableAnswer:
      'Trading AI bersifat eksperimental dan TIDAK dijamin menguntungkan. Futures kripto sangat volatil dan berisiko. AiT dirancang untuk tujuan edukasi dan riset. Kami sangat menyarankan: mulai dengan jumlah kecil (10-50 USDT), jangan investasi melebihi yang sanggup Anda rugi, uji sebelum trading nyata.',
    ...
    faqSupportedAIModelsAnswer:
      'AiT mendukung 7+ model AI: DeepSeek (direkomendasikan untuk biaya/performa), Qwen, OpenAI (GPT), Claude, Gemini, Grok, dan Kimi. Anda juga dapat menggunakan endpoint API yang kompatibel dengan OpenAI.',
    ...
    faqHowToInstallAnswer:
      'Metode termudah (Linux/macOS): Jalankan "curl -fsSL https://raw.githubusercontent.com/Aixxww/AiT/main/scripts/install.sh | bash" - ini menginstal kontainer Docker secara otomatis. Lalu buka http://127.0.0.1:3000 di browser Anda.',
    ...
    faqManualInstallationAnswer:
      'Prasyarat: Go 1.21+, Node.js 18+, TA-Lib. Langkah: 1) Clone repo; 2) "go mod download"; 3) "cd web && npm install"; 4) "go build -o ait"; 5) "./ait"; 6) "cd web && npm run dev".',
    ...
    faqUpdateAiTAnswer:
      'Docker: "docker compose pull && docker compose up -d". Manual: "git pull && go build -o ait" untuk backend, "cd web && npm install && npm run build" untuk frontend.',
    ...
    faqHyperliquidSetupAnswer:
      'Hyperliquid adalah bursa terdesentralisasi. Langkah: 1) Kunjungi app.hyperliquid.xyz; 2) Hubungkan wallet; 3) Buat API wallet; 4) Salin alamat dan private key; 5) Tambahkan di AiT. Tanpa KYC.',
    ...
    faqStopLossTakeProfit: 'Apakah AiT mendukung stop-loss dan take-profit?',
    faqStopLossTakeProfitAnswer:
    ...
    faqMultipleTradersAnswer:
      'Ya! AiT mendukung 20+ trader bersamaan. Gunakan untuk A/B test strategi, bandingkan model AI, atau diversifikasi lintas bursa.',
    ...
    faqPortInUseAnswer:
      'Periksa proses yang menggunakan port. Ubah port di .env: AiT_BACKEND_PORT=8081, AiT_FRONTEND_PORT=3001.',
    ...
    faqBinancePositionModeAnswer:
      'Anda dalam mode One-way tetapi AiT memerlukan Hedge Mode. Tutup semua posisi, beralih ke Hedge Mode, restart trader.',
    ...
    faqEncryptionDetails: 'Enkripsi apa yang digunakan AiT?',
    faqEncryptionDetailsAnswer:
      'AiT menggunakan: 1) AES-256-GCM untuk penyimpanan database; 2) RSA-2048 untuk enkripsi transport opsional; 3) JWT untuk token autentikasi.',
    ...
    faqCanAiTStealFunds: 'Bisakah AiT mencuri dana saya?',
    faqCanAiTStealFundsAnswer:
      'AiT open-source (lisensi AGPL-3.0) - Anda bisa audit semua kode. API key disimpan lokal di mesin ANDA, tidak pernah dikirim ke server eksternal.',
    ...
    faqCustomAIAPIAnswer:
      'Ya! AiT mendukung API yang kompatibel dengan OpenAI. Masukkan URL endpoint, API key, dan nama model.',
    faqAIHallucinations: 'Bagaimana dengan halusinasi AI?',
    faqAIHallucinationsAnswer:
      'AiT memitigasi dengan: prompt terstruktur, format output JSON, dan validasi order sebelum eksekusi. Namun trading AI tetap eksperimental.',
    ...
    faqHowToContributeAnswer:
      'AiT open-source dan menyambut kontribusi! Cara: 1) Kode - perbaiki bug, tambah fitur; 2) Dokumentasi; 3) Laporan Bug; 4) Ide Fitur. Semua kontributor mungkin mendapat reward airdrop.',
    faqPRGuidelines: 'Apa panduan PR?',
    faqPRGuidelinesAnswer:
      'Proses PR: 1) Fork repo; 2) Buat branch fitur dari dev; 3) Buat perubahan, jalankan lint; 4) Commit dengan format Conventional Commits; 5) Push dan buat PR ke Aixxww/AiT:dev.',
    ...
    environmentCheck: {
      ...
      unsupportedDesc:
        'Buka AiT melalui HTTPS (atau http://localhost saat pengembangan).',
    ...
    strategyStudio: {
      ...
      promptCompactMode: 'Ringkas data kandidat',
      promptCompactModeDesc:
        'Meringkas data pasar kandidat dalam prompt untuk menghemat token; matikan untuk mengirim detail lengkap.',
      promptCompactOn: 'Aktif',
      promptCompactOff: 'Mati',
      promptCompactReloadHint:
        'Perubahan tersimpan berlaku pada konfigurasi trader yang dimuat ulang; mulai ulang trader aktif untuk menerapkan segera.',
    },
    ...
    backtestChart: {
      ...
      interval: 'Interval',
      loadingKline: 'Memuat K-line...',
      lossClose: 'Tutup Rugi',
      noTrades: 'Tidak ada trade',
      openProfit: 'Profit Terbuka',
      symbol: 'Simbol',
      trades: 'Trade',
    },
    backtestOverview: {
    ...
    backtestPageExtra: {
      ...
      cancel: 'Batal',
      confirmDelete: 'Yakin ingin menghapus run ini?',
      delete: 'Hapus',
      equity: 'Ekuitas',
      maxDD: 'Max DD',
