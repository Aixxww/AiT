...
type AutoTraderConfig struct {
	...
	// Proxy configuration (per-exchange)
	ProxyURL string // Proxy URL (HTTP/HTTPS/SOCKS5)
...
type AutoTrader struct {
	...
	contextCacheMutex      sync.RWMutex             // Protects last successful account/position context snapshots
	lastContextBalance     map[string]interface{}   // Last successful account balance response
...
func NewAutoTrader(config AutoTraderConfig, st *store.Store, userID string) (*AutoTrader, error) {
	...
	case "binance":
		...
		trader = binance.NewFuturesTrader(config.BinanceAPIKey, config.BinanceSecretKey, userID, config.ProxyURL)
	case "bybit":
	...
	if st != nil && coinSrc.SourceType == "hunter_v7" {
		...
		v7OutcomeTracker.SetOutcomeCallback(func(outcome TrackedOutcome) {
			if err := st.HunterV7Signal().UpdateTrackOutcome(outcome.RecordID, store.HunterV7SignalTrackUpdate{
				...
				logger.Warnf("⚠️ [%s] Failed to update V7 tracking outcome: %v", config.Name, err)
			}
		})
	...
	return &AutoTrader{
		...
		v7OutcomeTracker:      v7OutcomeTracker,
	}, nil
}
...
func (at *AutoTrader) Run() (err error) {
	...
	if at.v7OutcomeTracker != nil {
		trackerCtx, trackerCancel := context.WithCancel(context.Background())
		at.v7OutcomeCancel = trackerCancel
		at.v7OutcomeTracker.Start(trackerCtx)
		at.monitorWg.Add(1)
		go func() {
			defer at.monitorWg.Done()
			<-at.stopMonitorCh
		...
		at.logInfof("📈 Hunter v7 signal outcome tracker enabled")
	}
...
func hunterV7TrackedCandlesSince(snapshotEngine *kernel.SnapshotEngine, symbol string, since time.Time) []TrackedCandle {
	...
	snap := snapshotEngine.GetSnapshot()
	if snap == nil || snap.Symbols == nil {
		return nil
	}
	ss := snap.Symbols[symbol]
	if ss == nil || len(ss.Klines) == 0 {
		return nil
	}
	klines := ss.Klines["1m"]
	if len(klines) == 0 {
