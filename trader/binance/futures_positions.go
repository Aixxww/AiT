...
func (t *FuturesTrader) GetPositions() ([]map[string]interface{}, error) {
	...
	positions, err := doSignedBinanceRequestWithRetry(t, "get positions", func() ([]*futures.PositionRisk, error) {
		return t.client.NewGetPositionRiskService().Do(context.Background(), futures.WithRecvWindow(binanceSignedRequestRecvWindow))
	})
	if err != nil {
		return nil, fmt.Errorf("failed to get positions: %s", sanitizeBinanceError(err))
	}
